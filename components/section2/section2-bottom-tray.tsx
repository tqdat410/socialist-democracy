...
import Section2DragCard from "@/components/section2/section2-drag-card";
import type { DragItem } from "@/lib/quiz-data";
import { useQuizStore } from "@/stores/quiz-store";

interface Section2BottomTrayProps {
  items: DragItem[];
  quiz: string;
}
...
export default function Section2BottomTray({ items, quiz }: Section2BottomTrayProps) {
        ...
        <SortableContext items={shuffledAvailable.map((i) => i.id)} strategy={rectSortingStrategy}>
          <div className="section1-bottom-tray-scroll">
            {shuffledAvailable.map((item) => (
              <Section2DragCard key={item.id} item={item} />
            ))}
          </div>
        </SortableContext>
      </div>
